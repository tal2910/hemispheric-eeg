...
async def _stream_shard_to_consumer(
    ...
    *,
    ...
    Exception policy: every failure mode produces accurate partial stats and
    a log line; we never let an exception escape this function. The
    orchestrator's integrity summary is what surfaces failures to the user.

      - ConnectionResetError / BrokenPipeError: consumer process dropped.
      - MemoryError: local out-of-memory during serialize/drain.
      - OSError: I/O failure on the mmap'd .npy (truncation, ENOSPC, EIO).
      - Anything else: logged with full traceback, treated like a fatal stream
        error. Stats still reflect what got through before the failure.
    """
    pool = _ArrayPool()
    sent = 0
    bytes_sent = 0
    ...
    try:
        ...
        log.error("port %d: out of memory after %d/%d chunks: %s",
                  port, sent, len(chunks), exc)
    except OSError as exc:
        # Disk read errors on the mmap'd .npy show up here (errno EIO, ENOSPC,
        # etc.). The visit's file may be corrupt or the filesystem unhealthy.
        log.error("port %d: I/O error after %d/%d chunks: %s",
                  port, sent, len(chunks), exc)
    except Exception:
        # Anything else (serialization bug, asyncio internal). Full traceback
        # in the log; stats still reflect what got through.
        log.exception("port %d: unexpected error after %d/%d chunks",
                      port, sent, len(chunks))
    finally:
        pool.close_all()
        try:
            writer.close()
    ...
    for shard, port in zip(shards, ports):
        ...
        def make_handler(s=shard, p=port, d=done, st=stats_by_port):
            async def handle(reader, writer):
                try:
                    st[p] = await _stream_shard_to_consumer(writer, s, port=p)
                finally:
                    d.set()
            return handle
