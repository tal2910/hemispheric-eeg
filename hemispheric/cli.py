...
def _write_run_report(
    *,
    ...
    """Write a machine-readable JSON summary of this run.

    Lives at ./run-reports/<timestamp>.json. Useful for:
      - Auditing: which filter was used, which visits matched, what got lost.
      - Forensics: per-port chunk counts plus, on failure, the list of
        affected visit_ids with how many chunks each lost.
      - Diffing across runs without grepping logs.
    """
    import json
    from datetime import datetime

    report_dir = Path("./run-reports")
    report_dir.mkdir(exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%dT%H%M%S")
    report_path = report_dir / f"{timestamp}.json"

    ports_data = []
    for s, shard in zip(stats, shards):
        port_record: dict = {
            "port": s.port,
            "chunks_planned": s.chunks_planned,
            "chunks_streamed": s.chunks_streamed,
            "bytes_streamed": s.bytes_streamed,
            "clean_close": s.clean_close,
        }
        if not s.is_complete:
            lost_chunks = shard[s.chunks_streamed:]
            visits_affected: dict[str, int] = {}
            for ref in lost_chunks:
                vid = ref.visit_id_bytes.hex()
                visits_affected[vid] = visits_affected.get(vid, 0) + 1
            port_record["loss"] = {
                "chunks_lost": s.chunks_lost,
                "visits_affected": [
                    {"visit_id": vid, "chunks_lost": count}
                    for vid, count in sorted(
                        visits_affected.items(), key=lambda kv: -kv[1]
                    )
                ],
            }
        else:
            port_record["loss"] = None
        ports_data.append(port_record)

    report = {
        "timestamp": datetime.now().isoformat(timespec="seconds"),
        "exit_code": exit_code,
        "integrity_ok": exit_code == 0,
        "filter": filter_desc,
        "visits_loaded": visits_loaded,
        "visits_matched": visits_matched,
        "chunks_planned": total_planned,
        "chunks_streamed": total_streamed,
...
async def _orchestrate(args: argparse.Namespace) -> int:
    ...
    # 0. Preflight: verify data dir is populated.
    check_data_dir(args.data_dir)

    # 1. Load visits. Corrupt/incomplete visits are quarantined here and
    ...
    # 2. Build visit_db.json from the surviving (clean) per-visit sidecars.
    #    Run AFTER load_all_visits so quarantined files don't end up in the
    #    registry the consumer reads.
    ...
    if provider_task.done() and not provider_task.cancelled():
        ...
        if stats:
            ...
            if all_clean:
                ...
                # Forensic breakdown: for each failed port, the chunks at
                # positions [streamed:] are the lost ones. Aggregate by visit
                # so the operator knows which subjects had data loss, not just
                # how many bytes were missed.
                for s, shard in zip(stats, shards):
                    if not s.is_complete:
                        lost_chunks = shard[s.chunks_streamed:]
                        visits_affected: dict[str, int] = {}
                        for ref in lost_chunks:
                            vid_hex = ref.visit_id_bytes.hex()
                            visits_affected[vid_hex] = visits_affected.get(vid_hex, 0) + 1
                        log.warning(
                            "  port %d: %d/%d chunks streamed (%d lost, clean_close=%s)",
                            s.port, s.chunks_streamed, s.chunks_planned,
                            s.chunks_lost, s.clean_close,
                        )
                        # Per-visit list, sorted by chunks lost (most impacted first).
                        # Show full list if <= 5 visits, else top 5 + count.
                        sorted_visits = sorted(
                            visits_affected.items(), key=lambda kv: -kv[1]
                        )
                        for vid_hex, lost in sorted_visits[:5]:
                            log.warning("    visit %s: %d chunk(s) lost", vid_hex, lost)
                        if len(sorted_visits) > 5:
                            log.warning(
                                "    ... and %d more visit(s) affected",
                                len(sorted_visits) - 5,
                            )
                exit_code = 2  # non-zero so CI can flag data loss

            # Always write a machine-readable run report. Reviewer can inspect
            # past runs, diff filters, or pipe into reporting tools.
            _write_run_report(
                stats=stats, shards=shards, total_planned=total_planned,
                total_streamed=total_streamed, total_bytes=total_bytes,
                filter_desc=visit_filter.describe(),
                visits_loaded=len(visits), visits_matched=len(matching),
                exit_code=exit_code,
            )
